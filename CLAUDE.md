...
> - **呈现层平滑:消除「持续掉帧感」**(`realtime_view.py` 的 `_install_frame_clock_keepalive`,2026-06-29):gtk4paintablesink 靠 video widget 的 **GdkFrameClock** 调度上屏;生产 realtime 视图没挂任何 tick callback → 帧时钟空转 → sink 回退到粗粒度定时等待(Windows 默认 **15.625ms**)→ 33.37ms/帧(29.97fps)在 15.6ms 网格上排成 **2×=31.25 / 3×=46.875ms 双峰**(实测 70%/16%)→ 每 6 帧顿一下。修复 = 给 widget 挂一个**常驻 no-op tick callback**(`add_tick_callback(lambda: GLib.SOURCE_CONTINUE)`)保活帧时钟,sink 改按刷新率排帧。实测 present 间隔 >43ms 卡顿帧 **16.0%→1.0%**、双峰消失,**跨平台**、与 CPU 无关。**实测推翻**了「AI 管线 GIL/CPU 争用饿死 GTK 主线程呈现」的旧说:CPU 线程从 ~20 砍到 ~4(torch/cv2/解码)对双峰**零影响**;旧 5 路 A/B「AI=on 唯一致抖」是**混淆变量**——对照基线脚本 `gtk_sink_baseline.py` 恰好一直挂着 tick callback、realtime 测时没挂,真正的变量是「帧时钟是否保活」而非 AI。埋点 `lada/gui/realtime/realtime_trace.py`(env `LADA_REALTIME_TRACE`,默认关零开销)+ 分析脚本 `scripts/analyze_present.py` 留作回归。详见 memory `realtime-judder-presentation-rootcause`。
>   - **后续(2026-06-29)分窗口复测 + 最大化对照**(`analyze_present.py` 现支持冷启动 vs 稳态分窗;autoplay hook 加 env `LADA_AUTOMAXIMIZE`):①**稳态(窗口化)已确认平滑** —— present 间隔 median 32.6ms(贴 33.37 目标)、stddev ~5ms、**单峰**(无 31/47 双峰)、%>43ms <1.5%、production 侧 process 2.5ms / AI 命中 100%,与正经播放器相当。②**冷启动**除首帧一次性 ~740ms 尖峰外同样平滑(AI 未就绪回退原片,production 仍 2.5ms),**冷启动卡顿不是 CPU/AI 争用**。③**最大化是真实的、可复现的呈现层退化**(非错觉):4 次配对(窗口化 vs 最大化各 2 次)稳态 **%frames>43ms ~1% → ~5%、>50ms 卡顿数 ~4×、p99 43ms(一帧)→ 50ms(翻倍帧)**;而 production 侧 process_ns / AI 命中**两种窗口完全相同** → 退化 100% 在 GTK 上屏/合成步骤,**与 AI 无关**,根因是 Windows 软件 sink 路径(#62 GL workaround)每帧重传 1080p + 按窗口面积重绘,面积越大越易错过 6.67ms vblank。④**GL 路径在本机不可用**:实验性强开 glsinkbin 直接 `failed to share contexts through wglShareLists` 报错、整条管线挂(0 帧上屏)—— 即 Win+Nvidia 上 GL 被**双重封死**(#62 颜色 + wglShareLists),软件路径是唯一可用项,故未保留任何强开 GL 的 flag。**结论:keepalive 已解决主要抖动;最大化残留是软件渲染路径的硬上限,本机无 app 层修法**(帧混合是错方向 —— 正经播放器默认也不开)。⑤**残留杠杆全部排除(2026-06-29)**:本机 **4K@150Hz scale=1**,最大化 = 1080p 上采样填 4K(~4× 像素)→ 软件路径每帧填/交换 4K 表面超 vblank。换渲染器无路(`GSK_RENDERER=vulkan` 本 GTK build 未编入,只有 `gl`/`cairo`,实测 3 变体最大化无差异);overlay 未逼离屏(控件 opacity 淡出、GTK 跳过 opacity=0);显示缩放已 1:1。**只剩 infra 级高成本项**,且其中最可能根治的 **GL sink 已实测确认 app 层修不了**(2026-06-29 time-box spike,GStreamer **1.26.10** + gtk4paintablesink **0.14.3**,均为当前版):强开 glsinkbin 全部失败于 gst-gl `glupload` 的 `wglShareLists 0xaa`(**ERROR_BUSY=170** —— glupload 自建 WGL context 去 share「正被 GTK 渲染线程占用」的 GTK WGL context);`GST_GL_PLATFORM/WINDOW/API`(wgl/egl/opengl/opengl3)各组合同错,**非旧版本问题** → 是 gst-gl 的 Windows 交互 bug,要改 GStreamer C 代码、非 app/env 可达(正是 upstream #62 撞的墙)。另一条 Vulkan GTK(gvsbuild 重编,未必有效)也属 infra。残留不大(~5% 帧略长、~5s 一次 hitch),**结论:呈现层到顶,maximized 残留接受;真要再投入,价值更高的是 seek 卡顿 #2/#3**。
