...
#
# Core of the BasicVSR++ TensorRT split: three wrappers that carve the network
# into TRT-compilable subgraphs (deform_align+backbone loop body, fused
# preprocess with an unrolled SPyNet, and the upsample tail), plus the runtime
# class that stitches the compiled engines back into a forward(lqs) drop-in.
#
# Ported from jasna (github.com/Kruk2/jasna). The three wrappers reach for the
# same attributes lada's BasicVSRPlusPlusNet exposes (spynet / feat_extract /
# deform_align[dir] / backbone[dir] / reconstruction / upsample1 / upsample2 /
# conv_hr / conv_last), so they attach to lada's model unchanged. Imports were
# repointed jasna -> lada; the bodies are verbatim.
...
from lada.trt.torch_tensorrt_export import (
...
# DIRECTIONS imported from trt_engine_paths
FEATURE_SIZE = 64
INPUT_SIZE = 256
